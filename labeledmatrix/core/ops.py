...
def hstack(list_of_lms: List[LabeledMatrix], sparse: bool = True):
    """
    >>> from labeledmatrix.core.labeledmatrix import LabeledMatrix
    ...
    True
    """
    total_rows = list_of_lms[0].row
    row_deco = {}
    for lm in list_of_lms[1:]:
        total_rows = total_rows.union(lm.row)[0]
        row_deco.update(lm.row_deco)
    aligned_matrix_list = []
    for lm in list_of_lms:
...
def reduce_sum(list_of_lms: List[LabeledMatrix]):
    """
    >>> from labeledmatrix.core.labeledmatrix import LabeledMatrix
    >>> import numpy as np
    ...
    total_rows, total_columns = list_of_lms[0].row, list_of_lms[0].column
    row_deco, column_deco = list_of_lms[0].deco
    for lm in list_of_lms[1:]:
        total_rows = total_rows.union(lm.row)[0]
...
def reduce_product(list_of_lms: List[LabeledMatrix]):
    """
    >>> from labeledmatrix.core.labeledmatrix import LabeledMatrix
    >>> from labeledmatrix.core.utils import aeq
    >>> import numpy as np
